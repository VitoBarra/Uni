...
static inline void WaitToAzzeration(BQueue_t *q) { WAIT(&q->azzeration, &q->m); }

static inline void SignalProducer(BQueue_t *q) { SIGNAL(&q->cfull); }
static inline void SignalConsumer(BQueue_t *q) { SIGNAL(&q->cempty); }
static inline void SignalAzzeration(BQueue_t *q) { SIGNAL(&q->azzeration); }
